...
import {deleteHouse, getAllHouse} from "../utils/ApiFunctions"
...
const ExistingHouses = () => {
    ...
    const fetchHouses = async () => {
        setIsLoading(true)
        try {
            const result = await getAllHouse()
            setHouses(result)
            setIsLoading(false)
        } catch (error) {
    ...
    const handleDelete = async (houseId) => {
        try {
            ...
            if (result === "") {
                setSuccessMessage(`House No ${houseId} was delete`)
                fetchHouses()
            } else {
    ...
    const currentRooms = filteredHouses.slice(indexOfFirstHouse, indexOfLastHouse)
                    ...
                    <section className="mt-5 mb-5 container">
                        ...
                        <Row>
                            <Col md={6} className="mb-2 md-mb-0">
                                <RoomFilter data={houses} setFilteredData={setFilteredHouses()}/>
                            </Col>
                        ...
                        <table className="table table-bordered table-hover">
                            ...
                            {currentRooms.map((house) => (
                                <tr key={house.id} className="text-center">
                                    ...
                                    <td className="gap-2">
                                        <Link to={`/edit-room/${house.id}`} className="gap-2">
